...
select distinct temp.hn,
	   ...
	   temp.fee_amount,
	   temp.mycount,
	   (select top 1 discharge_date_time
	   from patient_discharge_nl_view
	   where patient_visit_id = temp.patient_visit_id)  as [Discharge Date]
from 
(
...
--order by d.display_name_e
) as temp
where 
--temp.code = 'a09'
temp.Diagnosis = 'malignant neoplasm'
order by temp.display_name_e