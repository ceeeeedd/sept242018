--Volume and Revenue per Policy per Specialty with IPD/OPD discount 
...
(
				...
				SELECT DISTINCT ar.policy_id as [Policy ID]
							...
							,format(ar.transaction_date_time,'hh:mm tt') as [Time]
				...
				where --ar.policy_id =  '7D59250E-6ED6-11E3-84F9-78E3B58FDD66' --2014 INTELLICARE
							--AND ar.transaction_date_time  BETWEEN '05/01/2018 00:00:00:000' AND '05/31/2018 23:59:59:998'
							  cast(ar.transaction_date_time as date) between @From and @To
							 AND spsr.name_l in (@Payment_Status)
							 AND dpr.discount_posting_rule_id in ('05F8582D-6E0D-11E3-A6A2-78E3B597EAF4', '3D06C575-6E0D-11E3-A6A2-78E3B597EAF4')
