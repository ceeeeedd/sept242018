SELECT DISTINCT  
	   ...
	   --temp.expected_discharge_date_time as [Expected Discharge Date],
	   --format(temp.expected_discharge_date_time, 'hh:mm tt') as [Expected Discharge Time],
	   ...
	   --temp.recorded_at_date_time as [Recorded At Date],
	   --format(temp.recorded_at_date_time,'hh:mm tt') as [Recorded At Time],
	   ...
	   END as bmi
	   ,case when (temp.age = per.Age and temp.sex = per.Gender and (case when (temp.patient_height > 0 and temp.patient_weight > 0) then cast(ROUND(temp.patient_weight / ((temp.patient_height / 100) * (temp.patient_height / 100)),2) as numeric(12,1)) else 0 end) = per.BMI)
				  then per.Percentile
			 when (temp.age = per.Age and temp.sex = per.Gender and (case when (temp.patient_height > 0 and temp.patient_weight > 0) then cast(ROUND(temp.patient_weight / ((temp.patient_height / 100) * (temp.patient_height / 100)),2) as numeric(12,1)) else 0 end) > per.BMI and per.Percentile ='99th')
				  then 'Greater than 99th'
			 when (temp.age = per.Age and temp.sex = per.Gender and (case when (temp.patient_height > 0 and temp.patient_weight > 0) then cast(ROUND(temp.patient_weight / ((temp.patient_height / 100) * (temp.patient_height / 100)),2) as numeric(12,1)) else 0 end) < per.BMI and per.Percentile ='1st')
				  then 'Less than 1st'
			 --when (temp.age = per.[FEMALE AGE] and temp.sex = 'Female' and (case when (temp.patient_height > 0 and temp.patient_weight > 0) then cast(ROUND(temp.patient_weight / ((temp.patient_height / 100) * (temp.patient_height / 100)),2) as numeric(12,1)) else 0 end) = per.[FEMALE BMI])
				--  then per.[FEMALE PERCENTILE]
			 --when (temp.age = per.[FEMALE AGE] and temp.sex = 'Female' and (case when (temp.patient_height > 0 and temp.patient_weight > 0) then cast(ROUND(temp.patient_weight / ((temp.patient_height / 100) * (temp.patient_height / 100)),2) as numeric(12,1)) else 0 end) > per.[FEMALE BMI] and per.[FEMALE PERCENTILE] ='99th')
				--  then 'Greater than 99th'
			 --when (temp.age = per.[FEMALE AGE] and temp.sex = 'Female' and (case when (temp.patient_height > 0 and temp.patient_weight > 0) then cast(ROUND(temp.patient_weight / ((temp.patient_height / 100) * (temp.patient_height / 100)),2) as numeric(12,1)) else 0 end) < per.[FEMALE BMI] and per.[FEMALE PERCENTILE] ='1st')
				--  then 'Less than 1st'
			
				  end as percentile
	   ,case when temp.age <= 18 then 'Pedia' 
			 when temp.age >= 61 then 'Geriatric' else 'Adult' end as [Age Group]
...
--inner JOIN AHMC_DataAnalyticsDB.dbo.Percentiles per on temp.age = per.Age
where year(temp.actual_visit_date_time) = 2018
and month(temp.actual_visit_date_time) = 8
    --  year(temp.start_date_time) = 2014
	and temp.address_type in ('H1','N/A')
	and temp.age <= 18
	--and temp.display_name_l = 'De Guzman, Cody Gabriel'
